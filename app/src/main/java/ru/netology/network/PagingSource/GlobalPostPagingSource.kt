...
class GlobalPostPagingSource(
    private val api: GlobalWallApi,
    ...
    override suspend fun load(params: LoadParams<Long>): LoadResult<Long, PostDto> =
        withContext(Dispatchers.IO) {
            try {
                ...
                if (key == null) {
                    // Первая загрузка: свежие посты
                    val posts = api.getLatestPosts(count = initialCount)
                    ...
                    val posts = api.getPostsBefore(postId = key, count = params.loadSize)
